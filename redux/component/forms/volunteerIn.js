/**
 * Form for volunteers to sign in.
...
const Volunteer = React.createClass({
    ...
    getInitialState(){
        return {english: true, formValidated: true, submitted: false,
            phone: '', first: '', last: ''};
    },
    setLanguageState(){
    ...
    validatePhone(){
        ...
        var phone = document.getElementById("phoneVolunteer").value;
        if (phone.length < 10){
    ...
    validateFirstName(){
        this.setState({formValidated: true, submitted: false});
        var first = document.getElementById("firstNameVolunteer").value;
        if (first.length == 0){
    ...
    validateLastName(){
        this.setState({formValidated: true, submitted: false});
        var last = document.getElementById("lastNameVolunteer").value;
        if (last.length == 0){
    ...
    handleSubmit(e) {
        ...
        if ((this.state.phone.length && this.state.first.length  && this.state.last.length) != 0){
            var email = document.getElementById("emailVolunteer").value;
            var program = document.getElementById("programVolunteer").value;
            //noinspection JSUnresolvedFunction
            this.props.volunteerIn(this.state.first, this.state.last, this.state.phone, email, program);
    ...
    showError(){
        ...
        if (this.state.submitted){
            ...
            //return <Alert bsStyle="success">Welcome to BPSOS! <strong>Please remember to sign-out before leaving!</strong></Alert>
        }
    },
    renderPrograms() {
    ...
    resetForm(){
        const fields = ['firstNameVolunteer', 'lastNameVolunteer', 'emailVolunteer', 'phoneVolunteer'];
        fields.map(field => {
    ...
    render() {
        ...
        return (
            <Panel header="Volunteer/Guests Sign-In" bsStyle="primary">
                <br/>
                { this.showError() }
                <Form horizontal onSubmit={ this.handleSubmit }>
                    <FormGroup controlId="firstNameVolunteer" bsSize="large" onBlur={() => this.validateFirstName()}>
                    ...
                    <FormGroup controlId="lastNameVolunteer" bsSize="large" onBlur={() => this.validateLastName()}>
                        <Col componentClass={ControlLabel} sm={3}>
                            { last }
                        </Col>
                        <Col sm={9}>
                    ...
                    <FormGroup controlId="emailVolunteer" bsSize="large">
                        <Col componentClass={ControlLabel} sm={3}>
                            { email }
                        </Col>
                        <Col sm={9}>
                    ...
                    <FormGroup controlId="phoneVolunteer" bsSize="large" onBlur={() => this.validatePhone()}>
                        <Col componentClass={ControlLabel} sm={3}>
                            { phone }
                        </Col>
                        <Col sm={9}>
                    ...
                    <FormGroup controlId="programVolunteer" bsSize="large">
                        <Col componentClass={ControlLabel} sm={3}>
                            { program }
                        </Col>
                        <Col sm={9}>
